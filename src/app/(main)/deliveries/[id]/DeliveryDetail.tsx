...
import { Loader2, ChevronLeft, Pencil, Check, X, AlertCircle } from 'lucide-react'
...
export default function DeliveryDetail({ delivery, role }: { delivery: Delivery; role: string }) {
  ...
  const [editItems, setEditItems] = useState<EditableItem[]>(
    (delivery.delivery_items ?? []).map(item => ({
      ...item, editQty: String(item.quantity), editPrice: String(item.unit_price), isEditing: false
    }))
  )

  const canConfirm = ['store_manager', 'boss', 'receiver'].includes(role) && delivery.status === 'pending'

  function startEdit(id: string) { setEditItems(prev => prev.map(i => i.id === id ? { ...i, isEditing: true } : i)) }
  function cancelEdit(id: string) { setEditItems(prev => prev.map(i => i.id === id ? { ...i, editQty: String(i.quantity), editPrice: String(i.unit_price), isEditing: false } : i)) }
  function saveEdit(id: string) {
    setEditItems(prev => prev.map(item => {
      if (item.id !== id) return item
      const qty = parseFloat(item.editQty), price = parseFloat(item.editPrice)
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      toast.success(hasEdits ? '✅ 已修改并确认收货' : '✅ 已确认收货', { duration: 4000, style: { background: '#111', color: '#fff' } })
      router.refresh()
  ...
  async function handleReject() {
    ...
    try {
      ...
      if (result.error) { toast.error(result.error); return }
      toast('已驳回送货单', { icon: '❌', duration: 4000 })
      setShowReject(false); router.refresh()
    } finally { setLoading(false) }
  }
  ...
  return (
    <div className="space-y-4 pb-36">
      ...
      <div className="flex items-center gap-2 py-2">
        ...
        <span className={`ml-auto text-xs font-medium px-2.5 py-0.5 rounded-full ${getDeliveryStatusStyle(delivery.status)}`}>
      ...
      {canConfirm && (
        <div className="bg-orange-50 border border-orange-200 rounded-2xl px-4 py-3 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-orange-700">待您确认收货</p>
            <p className="text-xs text-orange-500 mt-0.5">请核对商品数量后确认，如有异议可修改或驳回</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden divide-y divide-gray-50">
        {[
          ['供应商', delivery.supplier?.name],
          ['配送门店', delivery.store?.name],
          ['送货日期', formatDate(delivery.delivery_date)],
          ['提交时间', formatDateTime(delivery.created_at)],
          ...(delivery.confirmed_at ? [[delivery.status === 'rejected' ? '驳回时间' : '确认时间', formatDateTime(delivery.confirmed_at)]] : []),
          ...(delivery.notes ? [['备注', delivery.notes]] : []),
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-50 flex items-center justify-between">
          ...
          {canConfirm && <span className="text-xs text-gray-400">✏️ 可修改数量</span>}
        </div>
              ...
              {item.isEditing ? (
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1">
                      ...
                      <input type="number" inputMode="decimal" value={item.editQty}
                        onChange={e => setEditItems(prev => prev.map(i => i.id === item.id ? { ...i, editQty: e.target.value } : i))}
                        autoFocus className="w-full mt-1 border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-gray-900 text-center" />
                    </div>
                    <div className="flex-1">
                      <label className="text-xs text-gray-400">单价（元）</label>
                      <input type="number" inputMode="decimal" value={item.editPrice}
                        onChange={e => setEditItems(prev => prev.map(i => i.id === item.id ? { ...i, editPrice: e.target.value } : i))}
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {item.original_quantity !== null && item.original_quantity !== item.quantity && (
                        <span className="text-xs bg-orange-100 text-orange-600 px-1.5 py-0.5 rounded-md">已修改</span>
                      )}
                    </div>
                    <p className="text-xs text-gray-400 mt-0.5">
                      {item.quantity}{item.unit} × ¥{item.unit_price}
                      {item.original_quantity !== null && item.original_quantity !== item.quantity && (
                        <span className="ml-2 line-through text-gray-300">原 {item.original_quantity}{item.unit}</span>
                      )}
                    </p>
                  </div>
                  <div className="flex items-center gap-2 ml-3">
                    ...
                    {canConfirm && (
                      <button onClick={() => startEdit(item.id)} className="p-1 text-gray-300 hover:text-gray-500">
        ...
        <div className="px-4 py-3 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
          <div>
            ...
            {hasEdits && <span className="ml-2 text-xs text-orange-500">（已修改）</span>}
          </div>
        ...
        <div className="fixed bottom-16 left-0 right-0 z-30 px-4 pb-4 bg-gradient-to-t from-gray-50 via-gray-50/90 to-transparent pt-8 pointer-events-none">
          <div className="flex gap-3 max-w-2xl mx-auto pointer-events-auto">
            <button onClick={() => setShowReject(true)}
              className="flex-1 py-3.5 rounded-xl border-2 border-red-200 text-sm font-semibold text-red-500 active:bg-red-50">
              ❌ 驳回
            </button>
      ...
      {/* 驳回输入 */}
      {canConfirm && showReject && (
        <div className="fixed bottom-16 left-0 right-0 z-30 bg-white border-t-2 border-red-100 px-4 pt-4 pb-6">
          <div className="max-w-2xl mx-auto space-y-3">
            <p className="text-sm font-semibold text-red-600">❌ 填写驳回原因</p>
            <textarea value={rejectReason} onChange={e => setRejectReason(e.target.value)}
              placeholder="请说明驳回原因，将通知供应商重新提交..." rows={3} autoFocus
              className="w-full border-2 border-red-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-red-400 resize-none placeholder:text-gray-300" />
            <div className="flex gap-3">
              <button onClick={() => { setShowReject(false); setRejectReason('') }}
                className="flex-1 py-3 rounded-xl border border-gray-200 text-sm font-medium text-gray-600">取消</button>
      ...
      {zoomedPhoto && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4" onClick={() => setZoomedPhoto(null)}>
          <div className="relative w-full max-w-sm aspect-square">
            <Image src={zoomedPhoto} alt="送货照片" fill className="object-contain" />
          </div>
