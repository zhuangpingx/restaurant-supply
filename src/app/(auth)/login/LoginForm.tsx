      ...
      {step === 'phone' ? (
        <>
          ...
          <button
            ...
            disabled={loading || phone.replace(/\s/g, '').length < 11}
            className={cn('w-full py-3.5 rounded-xl text-sm font-semibold transition-all bg-gray-900 text-white disabled:opacity-40 active:scale-[0.98]')}
          >
            {loading
              ? <span className="flex items-center justify-center gap-2"><Loader2 className="w-4 h-4 animate-spin" />发送中...</span>
              : '获取验证码'
            }
          </button>
        </>
        ...
        <>
            ...
            <div className="flex gap-2 justify-between" onPaste={handleOtpPaste}>
              {otp.map((digit, index) => (
                <input
                  ...
                  disabled={loading}
                  className={cn(
                    'w-12 h-12 text-center text-lg font-semibold rounded-xl border transition-all outline-none',
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <button
              ...
              className={cn('font-medium', countdown > 0 ? 'text-gray-300 cursor-not-allowed' : 'text-gray-900')}
            >
